...
const generateInvoiceFromString = async ({ dataBinding, options }) => {
  let { customerName, amount, products } = dataBinding;

  let start = now();

  // String concatination ile HTML olusturulur
  let invoiceTemplate = getTemplate(customerName, amount, products);
...
const generateInvoicePug = async ({ dataBinding, options }) => {
  const { customerName, amount, products } = dataBinding;
  let start = now();
...
const generateInvoiceVue = async ({ dataBinding, options }) => {
  ...
  const invoiceTemplate = fs.readFileSync(
    path.join(process.cwd(), "/templates/invoice.vue"),
    "utf8"
  );

  const app = createSSRApp({
    data: () => ({ customerName, amount, products }),
